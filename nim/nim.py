...
LIM_MIN, LIM_MAX = 1, 8
# ---------------------------------------------
DISPLAY_CAPTION = 'Das ganz grosse NIM-Spiel'
...
class Match_Area:
    def __init__(self, screen):
        self._screen = screen
        self._globs = { 'game': GA_INIT, 'row': NO_SELECT, 'count': 0, 'wins': [0, 0] }
        self._pics = []
        self._matches = []
        self._los = []
        self._actLos = []
        ...
        for x in range(PIKES * LIM_MAX):
            self._matches.append({'stat': STA_HIDDEN, 'pos': xyPos(x)})
        random.seed()
        self.newGame()  
    def draw(self):
        for x in range(PIKES * LIM_MAX):
    ...
    def newGame(self):
        if self._globs['game'] != GA_INIT:
            return
        pg.display.set_caption(DISPLAY_CAPTION)
        for x in range(PIKES * LIM_MAX):
            self._matches[x]['stat'] = STA_HIDDEN
        ...
        for i, v in enumerate(self._actLos):
            ...
            for x in range(b, b + v):
               self._matches[x]['stat'] = STA_ACTIV
        self._globs['game'] = GA_PLAY
    def playerSelect(self, pos):
        if self._globs['game'] != GA_PLAY:
            return
        ...
        if  sta > STA_INACT:
            ...
            if self._globs['row'] == row:   
                if sta == STA_ACTIV: 
                    self._matches[(self._globs['row'] * LIM_MAX + self._globs['count'])]['stat'] = STA_OPPONENT
                    self._globs['count'] += 1
                elif sta == STA_OPPONENT:
                    self._globs['count'] -= 1
                    self._matches[(self._globs['row'] * LIM_MAX + self._globs['count'])]['stat'] = STA_ACTIV
                    if not self._globs['count']:
                        self._globs['row'] = NO_SELECT
    def dealerSelect(self):
        if self._globs['game'] != GA_PLAY or self._globs['row'] == NO_SELECT:
            return
        ...
        if sum(x for x in self._actLos) == 0:
            ...
            self._globs['row'] = NO_SELECT
            self._globs['count'] = 0
            self._globs['game'] = GA_INIT
            self.setCaption()
            return
        self._globs['game'] = GA_DEAL    
    ...
    def result(self):
        ...
        for ind, val in enumerate(self._actLos):
            ...
            for x in range(ml, ol):
                self._matches[x]['stat'] = STA_INACT
